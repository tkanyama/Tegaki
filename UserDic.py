...
class UserDic(wx.Frame):
    def __init__(self, parent, id):
        wx.Frame.__init__(self, parent, id, 'ユーザ辞書の編集', size=(FRAME_WIDTH, FRAME_HEIGHT),
        ...
        with open(self.user_dic,encoding="utf-8_sig") as f:
            book = csv.reader(f)
        ...
        for i in range(col_n):
            if i == 0 or i >= 10:
                ...
                w = CELL_WIDTH
            self.grid.SetColLabelValue(i, chr(65 + i))
    ...
    def data_save(self):
        with open(self.user_dic, 'w', encoding="utf-8_sig", newline="\n") as f:
            writer = csv.writer(f)
            row_n = self.grid.GetNumberRows()
            col_n = self.grid.GetNumberCols()
            for i in range(row_n):
                col = []
                for j in range(col_n):
                    col.append(self.grid.GetCellValue(i, j))
                writer.writerow(col)
        self.Destroy()
